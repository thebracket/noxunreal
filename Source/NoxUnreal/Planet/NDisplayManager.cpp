...
void ANDisplayManager::BeginPlay()
{
	...
	for (size_t i = 0; i < nfu::CHUNKS_TOTAL; ++i) {
		...
		for (const auto &StaticModel : region->Chunks[i].static_voxel_models) {
			const int ModelId = StaticModel.Key;
			FString voxAddress;
			switch (ModelId) {
			...
			for (const auto &model : StaticModel.Value) {
				const int x = model.Get<0>();
				const int y = model.Get<1>();
				const int z = model.Get<2>();
				float mx = x + 0.5f;
				float my = y + 0.5f;
				float mz = z;

				const int model_layer = z - Chunks[i].base_z;
				...
				DynMaterial->SetVectorParameterValue("TintRGB", FLinearColor(1.0, 1.0, 1.0));
				model->SetMaterial(0, DynMaterial);
				model->SetRelativeLocation(FVector( mx * WORLD_SCALE, my * WORLD_SCALE, mz * WORLD_SCALE ));
				model->MarkRenderStateDirty();
				model->AttachTo(RootComponent);
				model->RegisterComponent();

				Chunks[i].layers[model_layer].StaticMeshes.Add(model);
			}
		}
